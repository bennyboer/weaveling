...
pub const PREFIX: &str = "piece_";

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct PieceId(Uuid);

#[derive(Debug, Clone, Copy, PartialEq, Eq, Error)]
#[error("not a valid piece id, expected a `{PREFIX}` prefix")]
...
mod tests {
    ...
    fn ids_generated_at_the_same_moment_still_differ() {
        assert_ne!(PieceId::generate(at(1_000)), PieceId::generate(at(1_000)));
    }
    ...
    fn display_and_parse_round_trip() {
        ...
        let parsed: PieceId = id.to_string().parse().expect("should parse back");
    ...
    fn parsing_rejects_non_uuid() {
        assert!("weaveling".parse::<PieceId>().is_err());
    }
    ...
    fn a_bare_uuid_is_not_accepted() {
        let bare = PieceId::generate(at(1_000)).as_uuid().to_string();

        assert!(
            bare.parse::<PieceId>().is_err(),
